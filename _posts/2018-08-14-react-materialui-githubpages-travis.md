...
Now that you have your App setup it's time to create some routes. The very important part here is to use `HashRouter` because github pages unfortunately doesnt work with `BrowserRouter`. If you'd like to further configure your router then refer to the [documentation](https://reacttraining.com/react-router/web/guides/philosophy).
...
Next click the settings of icon for that repo. Then setup the environment variable called `GITHUB_TOKEN`. If you don't have a github token, then read the previous step all the way through.  Make sure to keep the github token a SECRET by not displaying them in the logs! Mine look something like this
...
Next step is to setup your eslint rules for your editor. Without this step your editor will not give you the correct lint errors and warnings. We must extend `react-app` and a few more options depending on your liking. Mine looks something like this:
...
```json
  "scripts": {
    ...
    "prepush": "eslint src/**/*.js"
  },
```

Now, before you push your code, you will lint all your source code and eslint throw an error if any eslint errors occur and prevent the push. If the lint succeeds, then your push will continue.
